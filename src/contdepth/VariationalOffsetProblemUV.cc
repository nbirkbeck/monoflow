...
Vec2d VariationalOffsetProblemUV::defaultBeta(0.5, 0.5);
double VariationalOffsetProblemUV::defaultAlpha = 1.0;





VariationalOffsetProblemUV VariationalOffsetProblemUV::restriction(){
  int neww = (int)ceil(get_width()/2), newh = (int)ceil(get_height()/2);
  
  restrict = restriction_image(get_width(), get_height(), neww, newh);
    
  VariationalOffsetProblemUV sub(neww, newh);
  sub.S_I.clear();
...
robust_stats VariationalOffsetProblemUV::iteration(nacb::Imagef & residuals, //Input output
						   bool update){
